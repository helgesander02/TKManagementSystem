...
class edit_order(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.ph_label=customtkinter.CTkLabel(self.edit_top_, text="電話",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.phone=customtkinter.CTkEntry(self.edit_top_, placeholder_text="電話",fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.ph_label.grid(row=0,column=0,padx=30,pady=5)
        self.phone.grid(row=0, column=1,padx=30,pady=5)
        self.path_label=customtkinter.CTkLabel(self.edit_top_, text="通路",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.path=customtkinter.CTkComboBox(self.edit_top_,values=["","現場", "網站"],fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.path_label.grid(row=1,column=0,padx=30,pady=5)
        self.path.grid(row=1,column=1,padx=30,pady=5)
        self.pick_up_label=customtkinter.CTkLabel(self.edit_top_, text="取貨方式",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.pick_up=customtkinter.CTkComboBox(self.edit_top_,values=["","現場", "宅配"],fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.pick_up_label.grid(row=2,column=0,padx=30,pady=5)
        self.pick_up.grid(row=2,column=1,padx=30,pady=5)
        self.date_label=customtkinter.CTkLabel(self.edit_top_, text="日期",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.date_=DateEntry(self.edit_top_,selectmode='day',date_pattern='yyyy-mm-dd',font=("microsoft yahei", 22, 'bold'))
        self.date_1=DateEntry(self.edit_top_,selectmode='day',date_pattern='yyyy-mm-dd',font=("microsoft yahei", 22, 'bold'))
        ...
        self.money_label=customtkinter.CTkLabel(self.edit_top_, text="金額",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.money=customtkinter.CTkEntry(self.edit_top_, placeholder_text="",fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.money2=customtkinter.CTkEntry(self.edit_top_, placeholder_text="",fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        ...
        reset_bt=customtkinter.CTkButton(self.edit_top_,text='重新設定', width=150, height=40,
                                                        fg_color=("#5b5a5a"),
                                                        font=("microsoft yahei", 22, 'bold'),command=self.reset_
                                                        )
        ...
        self.page_=page_object(self)
        self.page_.page_max.configure(text=f'/{self.ol.page_max//20+1}')
        self.page_.pack(pady=20,side='bottom')
        def page_search(event):
            self.search_od_list(phone=self.phone.get(),pick_up=self.pick_up.get(),date_=self.date_.get_date(),date_1=self.date_1.get_date(),money1=self.money.get(),money2=self.money2.get(),path=self.path.get())
    ...
    def search_od_list(self,phone,path,pick_up,date_,date_1,money1,money2):
        self.ol.pack_forget()
        self.ol.destroy()
        self.ol=order_List(self,phone=phone,path=path,pick_up=pick_up,date_=date_,date_1=date_1,money1=money1,money2=money2,page=1 if self.page_.get()==None else self.page_.get(),fg_color = ("#DDDDDD"))
        self.ol.pack(fill='both',expand=1,padx=30,pady=5)
        self.page_.page_max.configure(text=f'/{self.ol.page_max//20+1}')
    def reset_(self):
...
class order_List(customtkinter.CTkFrame):
    ...
    def search(self):
        ...
        try:
            self.od_l={}
            order_list,self.page_max=search_od_(db=Session(engine),path=self.path,phone=self.phone,pick_up=self.pick_up,date_=self.date_,date_1=self.date_1,money1=self.money1,money2=self.money2,page=self.page)
            for i in order_list:
                    self.od_l[f'{i.id}']=[i.m_id_.Phone,i.id,i.pick_up_date,i.pick_up,','.join(list(map(lambda x:x.p_id_.product_Name,i.orders_))),i.pick_up_tf,i.total]
        ...
        a=customtkinter.CTkLabel(self.c,text='會員資訊',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=0)
        a=customtkinter.CTkLabel(self.c,text='訂單資訊',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=1)
        a=customtkinter.CTkLabel(self.c,text='取貨日期',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=2) 
        a=customtkinter.CTkLabel(self.c,text='取貨方式',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=3) 
        a=customtkinter.CTkLabel(self.c,text='訂單項目',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=4)
        a=customtkinter.CTkLabel(self.c,text='是否取貨',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=5)
        a=customtkinter.CTkLabel(self.c,text='金額',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=6)
        a=customtkinter.CTkLabel(self.c,text='編輯',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=7)
        a=customtkinter.CTkLabel(self.c,text='拆單',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=8)
        a=customtkinter.CTkLabel(self.c,text='刪除',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        a.grid(row=0,column=9)
        
        i=1
        def split_bill(i):return lambda:self.split_bill_(i)
        ...
        for key,value in self.od_l.items():
            ...
            a=customtkinter.CTkLabel(self.c,text=f'{value[2]}',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
            a.grid(row=i,column=2) 
            a=customtkinter.CTkLabel(self.c,text=f'{value[3]}',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
            ...
            a=customtkinter.CTkLabel(self.c,text=f'{value[5]}',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
            a.grid(row=i,column=5)
            a=customtkinter.CTkLabel(self.c,text=f'{value[6]}',fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
            a.grid(row=i,column=6)
            a=customtkinter.CTkButton(self.c,width=30,image=self.edit_photo,hover=False,text='',fg_color = ("#DDDDDD"),text_color='black',command=gen_cmd1(value[1]))
            a.grid(row=i,column=7)
            a=customtkinter.CTkButton(self.c,width=30,image=self.edit_photo,hover=False,text='',fg_color = ("#DDDDDD"),text_color='black',command=split_bill(value[1]))
            a.grid(row=i,column=8)
            a=customtkinter.CTkButton(self.c,width=30,image=self.delete_photo,hover=False,text='',fg_color = ("#DDDDDD"),text_color='black',command=gen_cmd(value[1]))#訂單號碼,ID
    ...
    def get_u(self,i):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            ...
            self.toplevel_window.focus()
    def split_bill_(self,i):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = split_bill_ToplevelWindow(self,id_=i)
            self.toplevel_window.sum_frame_.confirm_bt.configure(command=self.spilt_od)
            self.toplevel_window.sum_frame_.reset_bt.configure(command=self.spilt_od_reset)            
            self.toplevel_window.attributes('-topmost','true')   
        else:
            self.toplevel_window.focus()
    def spilt_od(self):
        try:
            spilt_bill_add(db=Session(engine),phone=self.toplevel_window.phone.get(),path=self.toplevel_window.path.get(),Pick_up=self.toplevel_window.pick_up.get(),remark=self.toplevel_window.Remark_textbox.get(1.0,'end'),product_=self.toplevel_window.buy_list,date_=self.toplevel_window.date_.get_date(),id_=self.toplevel_window.id_,discount=0 if self.toplevel_window.sum_frame_.discount_entry.get()=='' else self.toplevel_window.sum_frame_.discount_entry.get())
            self.toplevel_window.destroy()
            self.search()  
            tk.messagebox.showinfo(title='修改成功', message="修改成功", )
        except:
            tk.messagebox.showinfo(title='修改失敗', message="修改失敗", )
    def spilt_od_reset(self):
        discount=self.toplevel_window.sum_frame_.discount_entry.get()
        
        self.toplevel_window.sum_frame_.destroy()
        self.toplevel_window.sum_frame_=sum_Frame(self.toplevel_window.product_,a='',buy_list=self.toplevel_window.original_buy_list,bt_group=self.toplevel_window.bt_group,discount_=discount,  fg_color = ("#EEEEEE"),width=400)
        self.toplevel_window.sum_frame_.reset_bt.configure(command=self.spilt_od_reset)
        self.toplevel_window.sum_frame_.confirm_bt.configure(command=self.spilt_od)
        self.toplevel_window.sum_frame_.pack_propagate(0)
        self.toplevel_window.sum_frame_.pack(side='right',anchor='n',fill='both')                 
    def edit_(self,i):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = edit_ToplevelWindow(self,id_=i)
            self.toplevel_window.sum_frame_.confirm_bt.configure(command=self.edit_od)
            self.toplevel_window.sum_frame_.reset_bt.configure(command=self.edit_reset)  
            self.toplevel_window.attributes('-topmost','true')   
        else:
            self.toplevel_window.focus()
    def edit_od(self):
        try:
            edit_order_(db=Session(engine),phone=self.toplevel_window.phone.get(),path=self.toplevel_window.path.get(),Pick_up=self.toplevel_window.pick_up.get(),remark=self.toplevel_window.Remark_textbox.get(1.0,'end'),product_=self.toplevel_window.buy_list,date_=self.toplevel_window.date_.get_date(),id=self.toplevel_window.id_,discount=0 if self.toplevel_window.sum_frame_.discount_entry.get()=='' else self.toplevel_window.sum_frame_.discount_entry.get())
            self.toplevel_window.destroy()
            self.search()
            tk.messagebox.showinfo(title='修改成功', message="修改成功", )
        except Exception as e:
...
class edit_ToplevelWindow(customtkinter.CTkToplevel):
    def __init__(self,master, *args,id_, **kwargs):
        super().__init__(*args, **kwargs)
        self.title('編輯訂單')
        self.master=master
        self.buy_photo = customtkinter.CTkImage(light_image=Image.open(f"{os.getcwd()}\\image\\cart.png"),
                                  ...
                                  size=(30, 30))        
        od=get_edit_od(Session(engine),id_=id_)
        self.id_=id_
        ...
        self.ph_label=customtkinter.CTkLabel(self.input_top_, text="電話",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.phone=customtkinter.CTkEntry(self.input_top_, placeholder_text="電話",fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.phone.insert(tk.END,od.m_id_.Phone)
        self.phone.configure(state='disabled')
        self.ph_label.grid(row=0,column=0,padx=30,pady=5)
        self.phone.grid(row=0, column=1,padx=30,pady=5)
        self.path_label=customtkinter.CTkLabel(self.input_top_, text="通路",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.path=customtkinter.CTkComboBox(self.input_top_,values=["現場", "網站"],fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.path.set(od.path)
        self.path_label.grid(row=1,column=0,padx=30,pady=5)
        self.path.grid(row=1,column=1,padx=30,pady=5)
        self.pick_up_label=customtkinter.CTkLabel(self.input_top_, text="取貨方式",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.pick_up=customtkinter.CTkComboBox(self.input_top_,values=["現場", "宅配"],fg_color = ("#DDDDDD"),text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.pick_up.set(od.pick_up)
        self.pick_up_label.grid(row=2,column=0,padx=30,pady=5)
        self.pick_up.grid(row=2,column=1,padx=30,pady=5)
        self.date_label=customtkinter.CTkLabel(self.input_top_, text="取貨日期",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.date_=DateEntry(self.input_top_,selectmode='day',date_pattern='yyyy-mm-dd',font=("microsoft yahei", 22, 'bold'))
        self.date_.set_date(od.pick_up_date)
        self.date_label.grid(row=0,column=2,padx=30,pady=5)
        self.date_.grid(row=0,column=3,padx=30,pady=5)
        self.Remark_label=customtkinter.CTkLabel(self.input_top_, text="備註",text_color='black',font=("microsoft yahei", 22, 'bold'))
        self.Remark_label.grid(row=0,column=4,padx=30,pady=5)
        self.Remark_textbox = customtkinter.CTkTextbox(self.input_top_, corner_radius=0,fg_color='white',border_color='black',text_color='black',border_width=1,font=("microsoft yahei", 22, 'bold'))
        self.Remark_textbox.insert(tk.END,'' if od.remark==None else od.remark)
        ...
        self.sum_frame_=sum_Frame(self.product_,a='',buy_list=self.buy_list,bt_group=self.bt_group,discount_=od.discount,  fg_color = ("#EEEEEE"),width=400)
...
class split_bill_ToplevelWindow(customtkinter.CTkToplevel):
    def __init__(self,master, *args,id_, **kwargs):
        ...
        self.product_=customtkinter.CTkFrame(self, fg_color = ("#DDDDDD"))
        prodcuts=spilt_bill_pd(Session(engine),id_=self.id_)
        self.toplevel_window = None
        self.bt_group={}
        self.buy_list={}
        self.original_buy_list={}
        ...
        l=0
        for i in prodcuts:
            label_Name=customtkinter.CTkLabel(self.a_frame,text=i.p_id_.product_Name,text_color='black',font=("microsoft yahei", 22, 'bold'))
            label_Name.grid(row=l,column=0,padx=30,sticky='w')
            label_Weight=customtkinter.CTkLabel(self.a_frame,text=i.p_id_.product_Weight,text_color='black',font=("microsoft yahei", 22, 'bold'))
            label_Weight.grid(row=l,column=1,padx=30)
            label_price=customtkinter.CTkLabel(self.a_frame,text=f'{i.p_id_.product_Price}元',text_color='black',font=("microsoft yahei", 22, 'bold'))
            label_price.grid(row=l,column=2,padx=30)
            
            spinbox_1 = FloatSpinbox(self.a_frame, width=150, step_size=1)
            self.bt_group[i.p_id_.product_Name]=[spinbox_1,i.p_id_.product_Price]
            try:
...
class page_object(customtkinter.CTkFrame):
